...
public class SimpleJPQLTests {
    ...
    *  sql 보다 간결하며 틀정 dbms 에 의존하지 않는다.
    ...
    * */
    ...
    void Query를_이용한_다중행_조회_테스트(){
        ...
        Assertions.assertNotNull(resultMenuList);
        for(Object menu : resultMenuList) {
    ...
    void TypedQuery를_이용한_단일행_조회_테스트(){
        ...
        // 별칭 이름을 넣어야 조회 가능 (jpql 은 * 이 없다) (entity 이름도 안되고 꼭 별칭을 넣어줘야 한다.)
...
//        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class).getResultList();
        // 메소드 체이닝으로 축약가능 (두번째 인자의 유무로 Query 인지 , TypedQuery인지 구분)
    ...
    @Test
    void test2(){
        String jpql = "SELECT m FROM menu_section01 AS m WHERE m.menuName LIKE '%마늘%' OR m.menuName LIKE '마늘%' OR m.menuName LIKE '%마늘'";
        TypedQuery<Menu> query = entityManager.createQuery(jpql, Menu.class);
        List<Menu> menuList = query.getResultList();
        Assertions.assertNotNull(menuList);
        for(Menu menu : menuList) {
    ...
    *  이런 경우 Mybatis 는 명시적이고 복잡한 쿼리를 작성하는데 적합하며, 최적화에 유리하다.
    *
    *  Mybatis 는 복잡한 조인, 서브쿼리, 다양한 함수 등을 처리할 때 더 유연하고,
    *  쿼리의 복잡성을 완전히 개발자가 컨트롤할 수 있다.
    *  SQL 을 직접 작성하여 사용하므로, 객체 변환 없이 순수 SQL 을 실행할 수 있다.
    *  jpa 는 orm 을 통해 객체로 변환하는 과정으로 거쳐야 하고, 복잡해질수록 성능적 문제와 추가적인 시간이 소요된다.
    * */
