...
*
*  name : 매핑할 테이블의 컬럼 이름
*  insertable : 엔티티 저장시 필드 저장 여부 (default : true)
*  updatable : 엔티티 수정시 필드 저장 여부 (default : true)
*  table : 엔티티와 매핑될 테이블 이름. 하나의 엔티티를 두 개 이상의 테이블에 매핑할 때 사용
*  nullable : null 값 허용 여부 설정, not null 제약 조건에 해당함(default : true)
*  unique : 컬럼에 유일성 제약조건
*  length : 문자열 길이 , String type 에서만 사용( deafault : 255)
...
public class Member {
    ...
    @Transient // 테이블 생성시 무시. 자바에서는 필요한 값이지만 db 에 넣지 않을 때
