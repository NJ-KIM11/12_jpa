...
public class PrimaryKeyMappingTests {
    ...
    *  데이터 베이스마다 pk를 생성하는 방식이 서로 다르다.
    *  @GeneratedValue 는 다음과 같은 속성을 가지고 있다.
    *
    *  - strategy : 자동 생성 전략을 지정
    *  - GenerationType.IDENTITY : pk 생성을 데이터베이스에 위임( Mysql 의 AutoIncrement)
    *  - GenerationType.SEQUENCE : 데이터베이스 시퀀스 객체 사용 (ORACLE 의 SEQUENCE) , mysql 에는 없는 기능
    *  - GenerationType.TABLE : 키 생성 테이블 사용
    *  - GenerationType.AUTO : 자동 선택, db 에 따라서 자동 선택
    * */
    ...
    public void 식별자_매핑_테스트() {
        ...
        entityManager.persist(member);      // 한번에 하나만 가능 ( 여러번 써주던지 for 문 활용)
        entityManager.persist(member2);
        entityTransaction.commit();

        String jpql = "SELECT A.memberNo FROM member_section03 A";      // db 에 영향을 받지 않는 쿼리문 ( java 에서 쓰는 쿼리문 (자바종속형))
