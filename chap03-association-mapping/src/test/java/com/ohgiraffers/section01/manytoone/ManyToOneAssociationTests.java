...
public class ManyToOneAssociationTests {
    ...
    *  각 연관 관계 유형에 따라 jpa 가 데이터베이스와 상호작용 하는 방식이 달라지기 때문에
    ...
    * */
    ...
    void 다대일_연관관계_객체_삽입_테스트(){
        ...
        entityManager.persist(menuAndCategory);     // CascadeType.PERSIST 설정을 안해주면 연관관계를 가진 부모는(category) 영속화가 안되어 commit 안되고 rollback 됨
        entityTransaction.commit();                 // 위 설정을 해주어야 부모까지 영속화 되어 반영가능
    ...
    void detach_테스트(){
        ...
        * */
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class, 100000);
        menuAndCategory.setMenuName("변경함");
        Category category = menuAndCategory.getCategory();
        category.setCategoryName("no change??");
        menuAndCategory.setCategory(category);
        ...
        Category category1 = entityManager.find(Category.class, 33335);
