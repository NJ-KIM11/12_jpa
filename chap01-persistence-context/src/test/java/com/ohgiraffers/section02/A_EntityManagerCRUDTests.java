...
public class A_EntityManagerCRUDTests {
    ...
    public void 메뉴코드로_메뉴조회_테스트() {
        int menuCode = 1;
        Menu foundMenu = entityManager.find(Menu.class, menuCode);      // 두전째 인자로는 id(PK)로만 가능
    ...
    public void 새로운_메뉴_추가_테스트(){
        ...
        try {
            ...
            entityManager.persist(menu);        // 메모리 상에 저장
    ...
    public void 메뉴_이름_수정_테스트(){
        ...
        try{
            menu.setMenuName("쿠우쿠우골드");         // 트랜잭션 활성화 된 상태에서 entityManager 를 통해서 가져온 객체를 setter를 통해 바꿔주고 커밋해주면
            entityTransaction.commit();             // db 와 비교후 수정 반영
        } catch (Exception e) {
            entityTransaction.rollback();
            e.printStackTrace();
        }
    ...
    public void 메뉴_삭제하기_테스트(){
        Menu menuToRemove = entityManager.find(Menu.class,25);
        ...
        Menu removedMenu = entityManager.find(Menu.class,25);
