...
public class A_EntityLifeCycleTests {
    ...
    * */
    ...
    void 영속성_연속_조회_테스트(){
        ...
        * */
        Menu foundMenu1 = entityManager.find(Menu.class, 11);       // 영속 상태가 됨
        Menu foundMenu2 = entityManager.find(Menu.class, 11);       // select 문을 다시 날리지 않고 영속석 컨텍스트에서 영속 상태에 있는 객체를 고대로 꺼내온다.
    ...
    void 영속성_객체_추가_테스트(){
        ...
        Menu foundMenu = entityManager.find(Menu.class, menuToRegist.getMenuCode());    // db에는 반영이 안되어 있지만 영속 컨텍스트에서 가져올수 있다
    ...
    void 준영속_detach_테스트(){
        ...
        /*
        *  영속성 컨텍스트가 관리하던 엔티티 객체가 더 이상 관리되지 않는 상태로
        *  전환되면(detach), 해당 객체는 준영속 상태로 바뀐다.
    ...
    void close_테스트(){
        ...
        entityManager.close();     // 닫히면 다시 쓸 수 없다

        foundMenu1.setMenuPrice(5000);
        foundMenu2.setMenuPrice(5000);

        //@BeforEach 한것 처럼 다시 발급해야 사용가능
        Assertions.assertEquals(5000, entityManager.find(Menu.class,11).getMenuPrice());
        Assertions.assertEquals(5000, entityManager.find(Menu.class,12).getMenuPrice());
    ...
    void 삭제_remove_테스트(){
        ...
        * */
        ...
        // remove 했기 때문에 commit은 안했지만 어차피 지울 것이기 때문에 entityManager가 가져오지 않는다(null)
