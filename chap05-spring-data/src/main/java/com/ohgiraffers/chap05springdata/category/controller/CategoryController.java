...
@RestController
@RequestMapping("/category")    // REST API 설계시에는 *을 생략하는 것을 권장(경로가 꼬일수 있다)
public class CategoryController {
    ...
    @PostMapping("/update")
    public ResponseEntity<Object> updateCategory(@RequestBody HashMap<String,String> categoryToUpdate) {

        if(Objects.isNull(categoryToUpdate)){
            return ResponseEntity.status(404).body("이름과 코드를 입력해주세요.");
        }
        int categoryCode = Integer.parseInt(categoryToUpdate.get("categoryCode"));
        String categoryName = categoryToUpdate.get("categoryName");

        Category result = categoryService.updateCategory(categoryCode, categoryName);

        if(Objects.isNull(result)){
            return ResponseEntity.status(500).body("서버측 오류 : 없는 카테고리");
        }
        return ResponseEntity.ok(result);
    }

    @PostMapping("/delete")
    public ResponseEntity deleteCategory(@RequestBody HashMap<String,Integer> requestData) {
        Integer categoryCode = requestData.get("categoryCode");

        if(categoryCode == null){
            return ResponseEntity.status(400).body("카테고리 코드를 입력하세요");
        }

        boolean isDeleted = categoryService.deleteCategory(categoryCode);

        if(!isDeleted){
            return ResponseEntity.status(500).body("해당 카테고리를 찾을수 없습니다.");
        }
